...
- `pytfmbs.TFMBSLinear.telemetry_hint` mirrors the same metadata dictionary (layer name, sparsity, tile mask) so Torch/ONNX models can continuously emit compile-time hints that the runtime agent already expects.
