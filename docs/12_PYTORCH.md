...
1. `torch_to_tfmbs.py` exports a small PyTorch sequential model to ONNX, scans for `MatMul`/`Gemm` nodes, computes tile masks, and writes `tests/mlir/torch_tfmbs.mlir` with telemetry metadata (e.g., `{telemetry = {layer = "...", sparsity = 0.0000, fusion_order = [...], fusion_sparsity = ...}}`).
2. Feed the resulting MLIR into `mlir-opt` with `--load-dialect-plugin=build/libtfmbs_plugin.dylib --pass-pipeline="builtin.module(tfmbs-to-linalg)"` or the regression script `tests/mlir/run_tfmbs_to_linalg.py` to guarantee the `tfmbs` dialect lowers into `linalg.matmul` while telemetry attrs stay attached for the Adaptive Runtime Agent to consume later. The runtime already exposes the same keys via `pytfmbs.AdaptiveRuntimeAgent` so the agent can compare compile-time and runtime fusion hints and log them to `logs/adaptive_history.json`.
3. Run `tools/adaptive_dashboard.py` after executing workloads to correlate `fusion_order`/`fusion_sparsity` from `tests/mlir/torch_tfmbs.mlir` with the runtime history; the dashboard notes mismatches directly in the terminal output.
